...
{	
	...
	pfile = fopen(argv[2], "wb");      // read only
	if(NULL == pfile)
    {
	...
	unsigned char sbuf[2] = {SBEGIN, 0};      // Enable transmission,  do not verify

	if(RS232_SendBuf(com, sbuf, 2) != 2)
	{
...

  {
   ...
   for (int b=0; b<512; b++) {
     ...
     RS232_SendByte(com, BREAD);

     printf("%i\n",b);

     RS232_SendByte(com, b >> 8);
     RS232_SendByte(com, b & 0xff);


   unsigned short CheckSum = 0x0000;
   for(unsigned int i=0; i<512; i++) {
     while (!RS232_PollComport(com, &buf[i], 1)) {}
     CheckSum += buf[i];
     printf("%02X",buf[i]);
     if ((i % 32) == 31) printf("\n");
   }
  ...
  while (!RS232_PollComport(com, &temp, 1)) {}
  unsigned short CheckSum2 = temp << 8;
  while (!RS232_PollComport(com, &temp, 1)) {}
  CheckSum2 = CheckSum2 | temp;

  if (CheckSum != CheckSum2) { printf("%04X %04X",CheckSum ,CheckSum2); break; }

  while (!RS232_PollComport(com, &temp, 1)) {}
 

  fwrite(buf,1,512,pfile);
