...
{	
	...
	printf("Waiting for respond from Arduino...\n\n");
        ...
        RS232_SendByte(com, SERASE);

        while (!RS232_PollComport(com, &rx, 1)) {}
        printf("%i\n", rx);
	...
	while(!end) {
  	  ...
  	  if(BlkNum == 0) { printf("Uploading firmware...\n\n"); }
    	  DownloadProgress = 1;

           unsigned char buf[516];
 	  if((BlkNum == (BlkTot-1)) && (Remain != 0)) {
    		fread(buf, Remain, 1, pfile);
    		int filled = 512 - Remain;
    		int i = 0;
    		for(i; i<filled; i++) { buf[Remain+i] = 0xFF; }
      	    }
      	    else
      	    {
              fread(buf, 512, 1, pfile);
      	    }

             buf[512] = (BlkNum >> 8) & 0x00FF;
             buf[513] = BlkNum & 0x00FF;

             RS232_SendByte(com, SDATA);

             unsigned short CheckSum = 0x0000;
             for(unsigned int i=0; i<514; i++) { CheckSum += (unsigned char)buf[i]; }
             buf[514] = (CheckSum >> 8) & 0x00FF;
             buf[515] = CheckSum & 0x00FF;
 	     RS232_SendBuf(com, buf, 516);

             while (!RS232_PollComport(com, &rx, 1)) {}

 	     printf("%d  ", BlkNum);
             if (rx != SRSP) printf(" %i",rx);
